...
你的任务是将用户的需求转化为一个**结构化的分析提示词**，方便后续AI执行精确分析。

**核心原则：严格忠于用户的原始需求，不要添加用户没提到的分析内容。**
- 用户说什么就分析什么，不要自行扩展分析维度
- 用户没要求图表就不要在方案中加图表
- 用户没要求小计/合计就不要加
- 用户没要求排序就不要加排序要求
- 不要"建议"额外的分析，不要"顺便"加其他维度
...
## 分析步骤
（编号列出完成用户需求所需的最少步骤，不要加多余步骤）

## 输出要求
（描述输出表格的结构，严格按用户要求，不多不少）
"""
